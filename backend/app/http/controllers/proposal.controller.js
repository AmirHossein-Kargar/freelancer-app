...
class ProposalController extends Controller {
  ...
  async findProposalById(id) {
    if (!mongoose.isValidObjectId(id))
      throw createHttpError.BadRequest("The provided project ID is not valid");
    const proposal = await ProposalModel.findById(id);
     throw createHttpError.NotFound("Proposal not found.");
    return proposal;
  }
  async changeProposalStatus(req, res) {
    ...
    message = "Proposal status changed to rejected";
    if (status === 1)
      message = "Proposal status changed to pending approval";
