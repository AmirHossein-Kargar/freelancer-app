...
class UserController extends Controller {
  ...
  async verifyUser(req, res) {
    ...
    if (updateResult.modifiedCount === 0)
      throw createHttpError.InternalServerError("User status update failed");

    let message = "User status has been approved";
    if (status === 0) message = "User status has been changed to rejected";
    if (status === 1)
      message = "User status has been set to pending approval";
